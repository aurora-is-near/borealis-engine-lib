use aurora_refiner_lib::refiner_with_trait::{BlockWithMetadata, BlockWithMetadataWithTrait};
...
pub fn get_near_data_lake_stream_with_trait(
    block_height: u64,
    config: &DataLakeConfig,
    mut shutdown_rx: tokio::sync::broadcast::Receiver<()>,
) -> (
    tokio::sync::mpsc::Receiver<BlockWithMetadataWithTrait<NEARBlock, ()>>,
        ...
        loop {
            tokio::select! {
                Some(block) = stream.recv() => {
                    sender
                        .send(BlockWithMetadataWithTrait::new(convert(block), ()))
                        .await
