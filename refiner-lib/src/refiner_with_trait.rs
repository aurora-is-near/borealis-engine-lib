...
pub trait ToNearBlock {
    fn to_near_block(&self) -> aurora_refiner_types::near_block::NEARBlock;
}

/// Holds a block, typed as generic B, and metadata
#[derive(Debug)]
...
/// Holds a block, typed as type B that implements ToNearBlock, and metadata
#[derive(Debug)]
pub struct BlockWithMetadataWithTrait<B: ToNearBlock + Debug, M: Debug + Clone> {
    pub block: B,
    pub metadata: M,
}

impl<B: ToNearBlock + Debug, M: Debug + Clone> BlockWithMetadataWithTrait<B, M> {
    pub const fn new(block: B, metadata: M) -> Self {
        Self { block, metadata }
    }
}

pub async fn run_refiner<P: AsRef<Path> + Send, B: ToNearBlock + Debug, M: Debug + Clone + Send + Sync>(
    ctx: EngineContext,
    chain_id: u64,
    tx_storage_path: P,
    mut input: tokio::sync::mpsc::Receiver<BlockWithMetadataWithTrait<B, M>>,
        ...
        tokio::select! {
            ...
            maybe_message = input.recv() => {
                if let Some(message) = maybe_message {
                    let BlockWithMetadataWithTrait { block, metadata } = message;
                    let block = block.to_near_block();
