/// `GasLimit` represents the gas limit for an `eth_call` request.
///
/// The `gas_limit` field of `eth_call` is optional, but a `gas_limit` is still required by
/// Aurora Engine. Therefore, the two options for a gas limit are either `UserDefined`
/// (when the `gas_limit` is given in the `eth_call` request) or `Default`
/// (when `gas_limit` is not given in the request). A third option is given for the gas that
/// the user provided a value, which is higher than Borealis Engine's computational limit for
