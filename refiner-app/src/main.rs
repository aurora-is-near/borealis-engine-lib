...
async fn run_refiner_app(
    height: Option<u64>,
    total: Option<u64>,
    config: &config::Config,
) -> anyhow::Result<()> {
    let engine_account_id = config.refiner.engine_account_id.clone();
    ...
    fs::create_dir_all(engine_path)
        .map_err(|v| anyhow!("Unable to create or open directory {engine_path:?}, reason: {v}"))?;
